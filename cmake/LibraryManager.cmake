    ...
    if (NOT TARGET ${name}::${name})
        ...
        foreach (BLA_VENDOR ${vendors} "")
            ...
            if (${name}_FOUND)
                ...
                if ("${name}" STREQUAL "BLAS" AND "${BLA_VENDOR_FOUND}" STREQUAL "OpenBLAS") # FindBLAS doesn't set the include directory
                        if ("$ENV{HOMEBREW_PREFIX}" STREQUAL "")
                            set(ENV{HOMEBREW_PREFIX} "/usr/local")
                        endif ()
                        find_file(_private_cblas_h "cblas.h" PATHS "$ENV{HOMEBREW_PREFIX}/opt/openblas/include")
                        if (NOT _private_cblas_h)
                            message(FATAL_ERROR "Cannot find cblas.h in $ENV{HOMEBREW_PREFIX}/opt/openblas/include. Install openblas package from homebrew.")
                        endif ()
                endif()
                if ("${name}" STREQUAL "LAPACK")
                    target_compile_definitions(${name}::${name} INTERFACE -DEIGEN_USE_LAPACKE)
print_variable(BLA_VENDOR_FOUND)
                    if ("${BLA_VENDOR_FOUND}" STREQUAL "Apple" OR "${BLA_VENDOR_FOUND}" STREQUAL "OpenBLAS") # special subversion to use generic lapacke together with Accelerate or generic
                        if ("$ENV{HOMEBREW_PREFIX}" STREQUAL "")
                            set(ENV{HOMEBREW_PREFIX} "/usr/local")
                        endif ()
                        find_library(_private_lapacke_lib "lapacke" PATHS "$ENV{HOMEBREW_PREFIX}/opt/lapack/lib")
                        ...
                        find_file(_private_lapacke_h "lapacke.h" PATHS "$ENV{HOMEBREW_PREFIX}/opt/openblas/include")
