...
macro(__LibraryManager_findBLASorLAPACK name)
    ...
    if (NOT TARGET ${name}::${name})
        ...
        foreach (BLA_VENDOR ${vendors} "")
            ...
            message(STATUS "try BLA_VENDOR ${BLA_VENDOR} for ${name}")
            ...
            if (${name}_FOUND)
                message("Found ${name} with BLA_VENDOR=${BLA_VENDOR} ${name}_FOUND=${${name}_FOUND} ${name}_LIBRARIES=${${name}_LIBRARIES}")
                ...
                if (NOT BLA_VENDOR_FOUND STREQUAL "Apple")
                    target_compile_definitions(${name}::${name} INTERFACE "-DNO_USE_ACCELERATE")
                endif ()
                ...
                if (_MKLFOUND)
                    ...
                    if (DEFINED ENV{MKLROOT})
                        ...
                        find_path(MKL_INCLUDE_DIR "mkl.h" REQUIRED)
                ...
                if ("${name}" STREQUAL "BLAS")
                    if ("${BLA_VENDOR_FOUND}" STREQUAL "Apple" OR "${BLA_VENDOR_FOUND}" STREQUAL "OpenBLAS" OR BLAS_LIBRARIES MATCHES "libopenblas.") # special subversion to use generic openblas together with Accelerate or generic; FindBLAS doesn't set the include directory
                        if ("$ENV{HOMEBREW_PREFIX}" STREQUAL "")
                            set(ENV{HOMEBREW_PREFIX} "/usr/local")
                        endif ()
                        find_file(_private_cblas_h "cblas.h" PATHS "$ENV{HOMEBREW_PREFIX}/opt/openblas/include" "/opt/local/include/lapack")
                        if (NOT _private_cblas_h)
                            message(FATAL_ERROR "Cannot find cblas.h in $ENV{HOMEBREW_PREFIX}/opt/openblas/include. Install openblas package from homebrew or OpenBLAS-devel from macports.")

                        endif ()
                        string(REPLACE "/cblas.h" "" _private_cblas_include "${_private_cblas_h}")
                        target_include_directories(${name}::${name} INTERFACE ${_private_cblas_include})
                        target_compile_definitions(${name}::${name} INTERFACE -DNO_USE_ACCELERATE)
                    endif ()
                endif ()
                if ("${name}" STREQUAL "LAPACK")
                    ...
                    if ("${BLA_VENDOR_FOUND}" STREQUAL "Apple" OR "${BLA_VENDOR_FOUND}" STREQUAL "OpenBLAS" OR LAPACK_LIBRARIES MATCHES "liblapack.") # special subversion to use generic lapacke together with Accelerate or generic
                        ...
                        find_library(_private_lapacke_lib "lapacke" PATHS "$ENV{HOMEBREW_PREFIX}/opt/lapack/lib" "opt/local/lib" "/usr/lib" "/usr/lib/x86_64-linux-gnu")
                        if (NOT _private_lapacke_lib)
                            message(FATAL_ERROR "Cannot find lapacke library in $ENV{HOMEBREW_PREFIX}/opt/lapack/lib. Install openblas and lapack packages from homebrew.")
                        endif ()
                        find_file(_private_lapacke_h "lapacke.h" PATHS "$ENV{HOMEBREW_PREFIX}/opt/openblas/include" "/opt/local/include/lapack" "/usr/include")
                        if (NOT _private_lapacke_h)
                            message(FATAL_ERROR "Cannot find lapacke.h in $ENV{HOMEBREW_PREFIX}/opt/openblas/include. Install openblas and lapack packages from homebrew.")
                        endif ()
