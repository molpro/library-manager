...
function(LibraryManager_LAPACK target)
    message("LibraryManager_FindLAPACK for target ${target}")
    cmake_parse_arguments("ARG" "PRIVATE;INTERFACE;PUBLIC" "" "" ${ARGN})
    set(linkType "PUBLIC")
    if (ARG_PUBLIC)
        set(linkType "PUBLIC")
...
macro(__LibraryManager_findBLASorLAPACK name)
    ...
    if (TARGET ${name}::${name})
        message(STATUS "Target ${name}::${name} already exists")
    endif ()
    if (NOT TARGET ${name}::${name})
        ...
        if (DEFINED ENV{MKLROOT} AND NOT "$ENV{MKLROOT}" STREQUAL "${CMAKE_CURRENT_BINARY_DIR}/mkl") # FindLAPACK works better if the directory name is mkl. This is not the case for oneapi. When FindLAPACK catches up with this, this stanza can be removed.
            file(CREATE_LINK "$ENV{MKLROOT}" "${CMAKE_CURRENT_BINARY_DIR}/mkl" SYMBOLIC RESULT _mklroot)
            if (NOT _mklroot EQUAL 0)
                message(WARNING "Cannot create symlink for MKLROOT=$ENV{MKLROOT}; reason: ${_mklroot}")
            else ()
                set(ENV{MKLROOT} "${CMAKE_CURRENT_BINARY_DIR}/mkl")
            endif ()
        endif ()
        ...
        foreach (BLA_VENDOR ${vendors} "")
            ...
            if (${name}_FOUND)
                message(DEBUG "Found ${name} with BLA_VENDOR=${BLA_VENDOR}")
                ...
                string(REGEX MATCH "mkl_intel" _MKLFOUND "${${name}_LIBRARIES}")
                ...
                if (_MKLFOUND)
                ...
                else () # not Intel. Find the include directory.
                    message(DEBUG "Not Intel; ${name}_LIBRARIES=${${name}_LIBRARIES}")
                    if (${name} STREQUAL LAPACK)
                        target_compile_definitions(${name}::${name} INTERFACE "-DNO_USE_ACCELERATE") # because there is no LAPACKE support in Accelerate
                    endif ()
                    target_compile_definitions(${name}::${name} INTERFACE "-DNO_USE_MKL")
                    if (name STREQUAL "BLAS")
                        set(_HEADER "cblas.h")
                    else () # Lapack - get lapacke
                        set(_HEADER "lapacke.h")
                        foreach (_LIB IN LISTS ${name}_LIBRARIES)
                            if ("${_LIB}" MATCHES "liblapack.")
                                string(REPLACE "lapack" "lapacke" _LIB_LAPACKE "${_LIB}")
                                if (EXISTS "${_LIB_LAPACKE}")
                                    target_link_libraries(${name}::${name} INTERFACE "${_LIB_LAPACKE}")
                                endif ()
                            endif ()
                        endforeach ()
                    endif ()
                    message(DEBUG "${name}_LIBRARIES ${${name}_LIBRARIES}")
                    foreach (_LIB IN LISTS ${name}_LIBRARIES)
                        message(DEBUG "Trying _LIB=${_LIB}")
                        get_filename_component(_LIB_DIR "${_LIB}" DIRECTORY)
                        #                        file(REAL_PATH _INCLUDE_DIR "${_LIB}/../include")
                        get_filename_component(_PREFIX "${_LIB_DIR}" DIRECTORY)
                        message(DEBUG "Trying _PREFIX=${_PREFIX}")
                        set(_INCLUDE_DIR "${_PREFIX}/include")
                        if (EXISTS "${_INCLUDE_DIR}/${_HEADER}")
                            message(DEBUG "Found ${_INCLUDE_DIR}/${_HEADER}")
                            target_include_directories(${name}::${name} INTERFACE "${_INCLUDE_DIR}")
                            break()
                        endif ()
                    endforeach ()
                endif ()
