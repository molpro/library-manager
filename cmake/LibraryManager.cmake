    ...
    if (NOT TARGET ${name}::${name})
        ...
        foreach (BLA_VENDOR ${vendors} "")
            ...
            if (BLA_VENDOR STREQUAL "Apple" AND "${name}" STREQUAL "LAPACK")
                set(BLA_VENDOR "PKG_CONFIG") # Accelerate does not provide lapacke, so get it elsewhere
                set(ENV{PKG_CONFIG_PATH} "$ENV{PKG_CONFIG_PATH}:/opt/homebrew/opt/lapack/lib/pkgconfig:/usr/local/opt/lapack/lib/pkgconfig")
            endif ()
            ...
            if (BLA_VENDOR STREQUAL "PKG_CONFIG")
                ...
                if (${name} STREQUAL "LAPACK")
                    set(names "lapacke lapack")
                endif ()
                string(TOLOWER "${names}" names)
            ...
            if (${name}_FOUND)
                message(STATUS "Found ${name} with BLA_VENDOR=${BLA_VENDOR} ${name}_LIBRARIES=${${name}_LIBRARIES}")
